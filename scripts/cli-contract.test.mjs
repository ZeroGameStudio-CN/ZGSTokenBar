...
test('graduation verifies removed CLI routes stay unavailable', () => {
  assert.match(graduationGate, /'scripts\/retired-commands-acceptance\.mjs', join\(cliOutput, 'ZGSTokenBar\.Cli\.exe'\), retiredCommandsOutput/);
  assert.match(graduationGate, /const retiredCommandsOutput = join\(artifactsPath, 'retired-commands'\)/);
});
