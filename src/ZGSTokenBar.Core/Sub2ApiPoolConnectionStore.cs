...
public sealed record Sub2ApiPoolConnection(string Endpoint, string Token);

public interface ISub2ApiPoolConnectionStore
{
    Sub2ApiPoolConnection? Read();
    void Write(Sub2ApiPoolConnection connection);
    void Delete();
}

public static class Sub2ApiPoolEndpoint
{
...
public sealed class Sub2ApiPoolConnectionStore : ISub2ApiPoolConnectionStore
{
    public const string TargetName = "ZGSTokenBar:sub2api-pool";
    private const uint CredentialTypeGeneric = 1;
    private const uint CredentialPersistLocalMachine = 2;
    private const int ErrorNotFound = 1168;

    public Sub2ApiPoolConnection? Read()
    {
        ...
        try
        {
            ...
            if (!root.TryGetProperty("version", out var version)
                ...
                || !Sub2ApiPoolEndpoint.TryNormalize(endpointValue.GetString(), out var endpoint)
            ...
            return new Sub2ApiPoolConnection(endpoint, token);
        }
        catch (JsonException)
        {
            return null;
    ...
    public void Write(Sub2ApiPoolConnection connection)
    {
        if (!OperatingSystem.IsWindows()) throw new PlatformNotSupportedException("Sub2API pool credentials require Windows Credential Manager.");
        if (!Sub2ApiPoolEndpoint.TryNormalize(connection.Endpoint, out var endpoint))
        {
            throw new ArgumentException("The observer endpoint must use HTTPS, except for loopback development.", nameof(connection));
        }
        ...
        {
            throw new ArgumentException("The Sub2API pool observer token is invalid.", nameof(connection));
        }
